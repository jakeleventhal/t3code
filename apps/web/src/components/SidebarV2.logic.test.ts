...
describe("worktree lifecycle actions", () => {
  ...
  it("reports auto-settle enabled only when every member keeps it on", () => {
    const a = member("a");
    const b = member("b", { autoSettleDisabledAt: "2026-09-17T12:00:00Z" });
    expect(resolveWorktreeLifecycle([a], now).autoSettleEnabled).toBe(true);
    expect(resolveWorktreeLifecycle([a, b], now).autoSettleEnabled).toBe(false);
  });
