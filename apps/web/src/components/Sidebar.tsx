  ...
  const handleMultiSelectContextMenu = useCallback(
    async (position: { x: number; y: number }, worktreeMemberKeys?: ReadonlyArray<string>) => {
      ...
      const lifecycleMenu = buildThreadActionMenuItems({
        ...
        isRunning: false,
        supports: {
          // Auto-settle is a per-thread setting; the multi-selection menu
          // only carries lifecycle verbs.
      ...
      void (async () => {
        ...
        switch (clicked.value) {
          ...
          case "auto-settle:disabled": {
            const enabled = clicked.value === "auto-settle:enabled";
            for (const member of members) {
              if (member.archivedAt != null) continue;
              if ((member.autoSettleDisabledAt == null) === enabled) continue;
              const result = await setThreadAutoSettle(
                scopeThreadRef(member.environmentId, member.id),
                enabled,
              );
              if (result._tag === "Failure" && !isAtomCommandInterrupted(result)) {
                const error = squashAtomCommandFailure(result);
