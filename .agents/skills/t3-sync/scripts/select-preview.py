...
def preview_releases(releases):
    ...
    return sorted(candidates, key=lambda release: (release["published_at"], release["id"]), reverse=True)


def is_v2(sha):
    # A preview label alone does not establish that this is the V2 release train;
    # maintainers also cut previews from main.
    try:
        api(f"repos/{REPO}/contents/apps/server/src/persistence/initializeV2Database.ts?ref={sha}")
        return True
...
def main():
    ...
    skipped = []
    for release in preview_releases([release for page in pages for release in page]):
        commit = api(f"repos/{REPO}/commits/{release['tag_name']}")
        if is_v2(commit["sha"]):
            break
        skipped.append({"tag": release["tag_name"], "sha": commit["sha"], "reason": "not V2"})
    else:
        raise ValueError("No published V2 preview release found.")
    # Only non-V2 previews are skipped; an incomplete newest V2 preview still fails.
    result = snapshot(release, commit, args.desktop_arch)
    result["skipped_newer_previews"] = skipped
